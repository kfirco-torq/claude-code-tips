...
// Configuration
// Last CC release before the binary was code-split into Bun chunks (2.1.242+),
// which tweakcc's unpack/repack cannot reach.
const EXPECTED_VERSION = '2.1.241';
...
function createRegexPatch(find, replace) {
  ...
  // Brace-match to a bounded depth: an interpolation may be a ternary whose
  // branches are template literals holding further interpolations, and a flat
  // pattern would end the placeholder at the first inner `}`.
  const L0 = '[^{}]';
  const L1 = `(?:${L0}|\\$\\{${L0}*\\})`;
  const L2 = `(?:${L0}|\\$\\{${L1}*\\})`;
  const L3 = `(?:${L0}|\\$\\{${L2}*\\})`;
  const VAR_BODY = `${L3}*`;
  const varRegex = new RegExp(`\\$\\{${VAR_BODY}\\}`, 'g');
...
const patches = [
  ...
  { name: 'Remove Task tool Usage notes + examples (~2KB)',
    customRegex: /\n## Usage notes[\s\S]*?(\$\{[a-zA-Z_$]\w*\}\$\{[a-zA-Z_$]\w*\}\n\n\$\{[a-zA-Z_$]\w*\?[a-zA-Z_$]\w*:[a-zA-Z_$]\w*\})`\}/,
    customReplace: '\n$1`}' },
  ...
  { name: 'Slim Skill tool instructions (1.4KB → 160 chars)', file: 'skill-tool' },
  { name: 'Slim WebFetch usage notes (808 → 120 chars)', file: 'webfetch-usage' },
  { name: 'Slim Grep tool description (~715 → 350 chars)',
    customRegex: /A powerful search tool built on ripgrep\n\n  Usage:\n  - ALWAYS use (\$\{[^}]+\}) for search tasks\. NEVER invoke \\`grep\\` or \\`rg\\` as a (\$\{[^}]+\}) command\.[\s\S]*?multiline: true/,
    customReplace: 'A powerful search tool built on ripgrep\n\n  Usage:\n  - ALWAYS use $1 for search tasks. NEVER invoke \\`grep\\` or \\`rg\\` as a $2 command.\n  - Full regex syntax. Filter by glob ("*.js") or type ("js", "py")\n  - Output modes: "content" (lines), "files_with_matches" (paths, default), "count"\n  - Uses ripgrep syntax - escape literal braces (\\`interface\\\\{\\\\}\\`)\n  - For multiline patterns, use \\`multiline: true\\`' },
  ...
  { name: 'Slim Workflow tool description (19.5KB → ~0.7KB; manual moved to workflow-authoring skill)',
    customRegex: /Execute a workflow script that orchestrates multiple subagents deterministically\.[\s\S]*?hand-author a continuation script\./,
    customReplace: 'Execute a workflow script that orchestrates multiple subagents deterministically: JS with agent()/parallel()/pipeline()/phase() hooks that runs in the background and returns structured results. ONLY use when the user has explicitly opted into multi-agent orchestration (the keyword "ultracode", an explicit request to run a workflow or fan out agents, or a skill/command that calls Workflow); otherwise use the Agent tool or ask first. Before authoring a script you MUST load the "workflow-authoring" skill via the Skill tool for the full API, pipeline-vs-parallel rules, structured-output schemas, quality patterns, and guardrails. The script, scriptPath, name, and args parameters are documented below.' },
  ...
  // artifact-tool retired at 2.1.220: split into flag-gated pieces with new list/watch actions; needs a fresh port.
];
...
function main() {
  ...
  // Hash check disabled — we apply across versions
  console.log(`Backup found, applying patches...`);
